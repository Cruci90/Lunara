...
export default function CalendarTab({ data, onDayClick }) {
  ...
  // Mapa de "Y-M-D" → [foodId] para los alimentos introducidos
  const foodsByDate = {};
  for (const [foodId, entry] of Object.entries(data.foods)) {
    const dateStr = getFoodDate(entry);
    if (!dateStr) continue;
    ...
    <div>
      ...
      <div className="calendar-grid">
        ...
        {calendarDays.map((day, i) => {
          ...
          const hasAllergen = dayFoods.some((id) => {
            const f = getFood(id) ?? (data.customFoods ?? []).find((cf) => cf.id === id);
            return f?.al;
          });
              ...
              {dayFoods.length > 0 && (
                <div className="calendar-food-dots">
                  {dayFoods.slice(0, 4).map((id) => {
                    const f = getFood(id) ?? (data.customFoods ?? []).find((cf) => cf.id === id);
                    return <span key={id} className="calendar-food-dot">{f?.em ?? ""}</span>;
                  })}
                </div>
              )}
            </div>
      ...
      <div className="legend" style={{ marginTop: 16 }}>
        <span className="legend-item"><span className="legend-dot" style={{ background: "var(--ac)" }} /> Alimento</span>
        <span className="legend-item"><span className="legend-dot" style={{ background: "var(--or)" }} /> Alérgeno</span>
        <span className="legend-item"><span className="legend-dot" style={{ background: "var(--bl)" }} /> Hoy</span>
        <span className="legend-item"><span className="legend-dot" style={{ background: "var(--pu)" }} /> Comidas</span>
      </div>
    </div>
  );
}
